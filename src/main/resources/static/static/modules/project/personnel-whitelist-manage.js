    ...
    oTableInit.Init = function () {
        $('#dataList').bootstrapTable({
            ...
            columns: [
			{
				...
				valign : 'middle'
                // field: 'id'
                // <input type="checkbox" name="accIds" value="'+data.id+'"/>
	        },{
                field: 'id',
                title:'人员ID',
                align : 'center',
				...
				valign : 'middle',
                    formatter:function(value){
                        if(value==null){
                            ...
                            var  htmlStr = '<button class="btn btn-box-tool" type="button" style=" background: none; border:none;" onclick=""><i class="fa fa-check text-green"></i></button>';
                            return htmlStr;
                            // return "允许";
                        }else{
                            var operateHtml = '<button class="btn btn-box-tool" type="button" onclick="" style=" background: none; border:none;"><i class="glyphicon glyphicon-remove icon-remove" style="color:red;"></i></button>';
                            return operateHtml;
                            // return "禁止";
                        }
                    }
...
$('#toolbar button').on('click',function(){
	...
	if($(this).attr('id')=='btn_add'){
		/* $('#myform')[0].reset();//清空表单 */
		$('#addModal').find('.modal-title').html('新增业主人员信息');
...
//多选人员下发设备
function personsSendAtt() {
    alert("多选人员下发设备");
    var hasRow = false;
    $('#dataList').find("input[name='btSelectItem']").each(function(){
        if($(this).is(':checked')){
            hasRow = true;
        }
    });
    ...
    var personList = $('#dataList').find("input[name='btSelectItem']");
    for(var i=0;i<personList.length;i++){
        console.log(personList[i]);
        var parent = personList[i].parentNode;
        console.log(parent.getElementById());
...
function fnResetaddForm(){
	...
	$('#myform').find("input[name='userType']").val('04');
	$('#myform').find("input[name='photoStr']").val('');
...
var doEdit = function(id){
	fnResetaddForm();
	$('#addModal').find('.modal-title').html('编辑业主人员信息');
	$('#addModal').modal('toggle');
	$.ajax({
	...
	$.ajax({
		...
		success:function(json){
			if(json){
                ...
                $('#icNumberOut_div').html(personnel.icNumberOut);//IC卡外卡号填写
				if(personnel.photoStr!=null&&personnel.photoStr!=''){
